...
import com.utad.movies_kotlin.Data.Local.Database.Repositories.User_Repository
...
class User_ViewModel @Inject constructor(private val user_repository: User_Repository) :
    ...
    fun validateUser(user: User_Entitie, et_UserName: EditText, et_Password: EditText) {
        viewModelScope.launch {
            ...
            if (!isvalid) {
                when (errorMessage) {
                    "Error: User is requiered" -> et_UserName.error = errorMessage
                    "Error: Password is required" -> et_Password.error = errorMessage
                }
                return@launch
            }
            var userValid = user_repository.validateUser(user.username, user.email, user.password)
            if (userValid != null) {
                //añadir funcionalidad de navegacion
